...
# ----------------------------------------------------------------------------------------------- #
def ParseReactionList(reactionList, reactionArrow='->'):
	...
	sMatrixTKeyIndexed = zeros(len(dataSplit), dtype=dtypeArray)
...
# ----------------------------------------------------------------------------------------------- #
def Get_IO_Status_for_Compound_List(compoundList, ioStatusList):
	...
	ioStatusDict = {}
	i = 0
	while i < len(ioStatusList):
		entry = ioStatusList[i]
		compound = entry[0]
		ioStatus = entry[1]
		ioStatusDict[compound] = ioStatus
		i += 1
	
	i = 0
	while i < len(compoundList):
		compound = compoundList[i]	
		try:
			ioStatus = ioStatusDict[compound]
		except:
			pdb.set_trace()
		matchedIOStatusList.append(ioStatus)
		i += 1



	return matchedIOStatusList
